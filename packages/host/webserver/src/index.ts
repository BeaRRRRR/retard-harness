...
export class WebServer extends Service {
  ...
  async [Service.init](): Promise<void> {
    const handle = async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
      ...
      // --- retard-harness proxy: strips frame-blocking headers so any site
      //     can be embedded in the mini-browser iframe. ---
      if (rawPath === '/dino-proxy') {
        const target = rawUrl.searchParams.get('url')
        ...
        if (targetUrl.protocol !== 'http:' && targetUrl.protocol !== 'https:') {
          res.writeHead(400); res.end('only http/https'); return
        }
        try {
          ...
          // Buffer the full response so we can control headers and ensure
          // the iframe receives uncompressed content.
          const body = upstream.body !== null ? Buffer.from(await upstream.arrayBuffer()) : null
          const passHeaders: Record<string, string> = {}
          const skip = new Set(['x-frame-options', 'content-security-policy', 'content-security-policy-report-only', 'transfer-encoding', 'connection', 'content-encoding', 'content-length'])
          for (const [key, value] of upstream.headers.entries()) {
            if (!skip.has(key.toLowerCase())) passHeaders[key] = value
          }
          passHeaders['X-Frame-Options'] = 'ALLOWALL'
          passHeaders['Content-Security-Policy'] = 'frame-ancestors *'
          if (body !== null) passHeaders['Content-Length'] = String(body.length)
          res.writeHead(upstream.status, passHeaders)
          if (body !== null) res.end(body); else res.end()
        } catch (err) {
          res.writeHead(502); res.end('proxy error: ' + String(err))
        }
        return
      }
